...
/// <summary>
/// Sheet: UT06_SubmitQuiz — <see cref="SubmitQuizCommandHandler.Handle"/>
/// Module: Quiz · CC = 22 · 25 test case (mỗi đường thực thi độc lập 1 TC — spec §3.3)
///
...
///        B18 cập nhật tiến độ · B19 tạo progress mới · B20 idempotent tiến độ
///        B21 điểm tuyệt đối 100 ⇒ cộng điểm hành vi · B22 rollback
/// </summary>
public class UT06_SubmitQuiz_HandleTests
{
    ...
    public UT06_SubmitQuiz_HandleTests()
    {
        PrivateSetter.Set(_enrollment, nameof(Enrollment.Course),
    ...
    private SubmitQuizCommandHandler CreateSut() =>
        new(_uow.Object, Mock.Of<ILogger<SubmitQuizCommandHandler>>(), _behavior.Object);
    ...
    /// <summary>UTCID16 · B17=T, B21=T · Type B — 3/3 câu đúng ⇒ 100.00 (biên trên).</summary>
    [Fact]
    ...
    /// <summary>UTCID23 · B22=T · Type A — commit thất bại ⇒ rollback và ném lại exception.</summary>
    [Fact]
    ...
    /// <summary>
    /// UTCID25 · B21=T vs B21=F · Type B — chỉ điểm TUYỆT ĐỐI 100 mới được cộng điểm hành vi.
    /// So sánh trực tiếp hai lần chạy để chốt biên: 66.67 (đạt nhưng không tuyệt đối) ⇒ KHÔNG cộng,
    /// 100.00 ⇒ cộng đúng một lần.
    /// </summary>
    [Fact]
    public async Task UTCID25_OnlyPerfectScoreIncreasesBehaviorScore()
    {
        var quiz = BuildQuiz();
        SetupScenario(quiz, NewAttempt());

        await Act(BuildAnswers(quiz, correctCount: 2));

        _behavior.Verify(x => x.IncreaseScoreAsync(
            It.IsAny<Guid>(), It.IsAny<IEnumerable<Tag>>(), It.IsAny<int>(), It.IsAny<CancellationToken>()),
            Times.Never);

        // Lượt thứ hai trên enrollment sạch, lần này đúng toàn bộ.
        var perfectFixture = new UT06_SubmitQuiz_HandleTests();
        var perfectQuiz = BuildQuiz();
        perfectFixture.SetupScenario(perfectQuiz, perfectFixture.NewAttempt());

        await perfectFixture.Act(BuildAnswers(perfectQuiz, correctCount: 3));

        perfectFixture._behavior.Verify(x => x.IncreaseScoreAsync(
